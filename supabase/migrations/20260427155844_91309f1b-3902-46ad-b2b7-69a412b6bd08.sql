...
CREATE OR REPLACE FUNCTION public.get_materials_for_students()
 RETURNS TABLE(id uuid, title text, description text, file_name text, file_type text, file_url text, file_size integer, subject text, category text, lesson_number integer, author text, genre text, year integer, oficiu integer, timer_minutes integer, created_at timestamp with time zone, updated_at timestamp with time zone, publish_at timestamp with time zone, subject_config jsonb, chapter_id uuid, barem_url text, barem_name text, barem_size integer, study_classes text[])
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  RETURN QUERY
  SELECT 
    m.id, m.title, m.description, m.file_name, m.file_type, m.file_url, m.file_size,
    m.subject, m.category, m.lesson_number, m.author, m.genre, m.year, m.oficiu,
    m.timer_minutes, m.created_at, m.updated_at, m.publish_at,
    CASE 
    ...
    m.chapter_id, m.barem_url, m.barem_name, m.barem_size, m.study_classes
